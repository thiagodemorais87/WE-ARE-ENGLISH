    ...
    <article
      className={[
        ...
        light
          ...
          : 'border-edge bg-gradient-to-br from-white/[0.06] to-transparent hover:border-cherry/40 hover:bg-white/[0.08]',
      ].join(' ')}
    >
      <div className="mb-3 text-2xl">{category.icon}</div>
      <h3 className={['text-base font-semibold', light ? 'text-ink' : 'text-fg'].join(' ')}>
        {category.label}
      </h3>
      <p
        className={[
          'mt-2 flex-1 text-sm leading-relaxed line-clamp-3',
          light ? 'text-muted' : 'text-fg-muted',
        ].join(' ')}
      >
