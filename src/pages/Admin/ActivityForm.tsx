...
function defaultContent(type: ActivityType): ActivityContent {
  switch (type) {
    ...
    case 'fill_blank':
      return { text: 'I _____ a student.', blanks: [{ id: 'b1', answer: 'am' }] }
    case 'word_order':
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-3xl space-y-5">
      <label className="block space-y-1">
        <span className="text-xs uppercase text-white/45">Title</span>
        <input
          value={values.title}
          onChange={(e) => set('title', e.target.value)}
          className="w-full rounded-xl border border-white/15 bg-ink/50 px-3 py-2 text-white"
        />
      </label>
      <label className="block space-y-1">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <label className="block space-y-1">
          <span className="text-xs uppercase text-white/45">Type</span>
          <select
            value={values.type}
            onChange={(e) => set('type', e.target.value as EngineActivityType)}
            className="w-full rounded-xl border border-white/15 bg-ink px-3 py-2 text-white"
          >
            {ENGINE_TYPES.map((t) => (
        ...
        <label className="block space-y-1">
          <span className="text-xs uppercase text-white/45">Level</span>
          <select
            value={values.level}
        ...
        <label className="block space-y-1">
          <span className="text-xs uppercase text-white/45">Difficulty</span>
          <select
      ...
      <label className="block space-y-1">
        <span className="text-xs uppercase text-white/45">Instructions</span>
        <textarea
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block space-y-1">
          <span className="text-xs uppercase text-white/45">Duration (min)</span>
          <input
            type="number"
            min={1}
        ...
        <label className="block space-y-1">
          <span className="text-xs uppercase text-white/45">Points</span>
          <input
            type="number"
            min={1}
            value={values.points}
      ...
      <label className="block space-y-1">
        <span className="text-xs uppercase text-white/45">Content JSON</span>
        <textarea
          value={values.contentJson}
          onChange={(e) => set('contentJson', e.target.value)}
          rows={12}
          className="w-full rounded-xl border border-white/15 bg-ink/70 px-3 py-2 font-mono text-xs text-white"
        />
      </label>
      <label className="block space-y-1">
        <span className="text-xs uppercase text-white/45">Image URL</span>
        <input
      ...
      <div className="space-y-2">
        <label className="block space-y-1">
          <span className="text-xs uppercase text-white/45">Audio URL</span>
          <input
        ...
        {audioMsg ? <p className="text-xs text-white/50">{audioMsg}</p> : null}
      </div>
